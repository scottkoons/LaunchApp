...
separate `notifications/scheduler.ts` Worker invokes its protected dispatcher
every minute because the Sites deployment manifest does not expose cron
triggers.
...
subscriptions continue working. `workers_dev: false` exposes no public scheduler
endpoint. The dispatcher additionally requires the independent secret in the
